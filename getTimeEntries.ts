...
/**
 * https://avanti.stoplight.io/docs/avanti-api/4952dd2917595-list-time-entries
 * @param viewId
 * @param templateId
 * @param parameters
 * @param credentials
...
export async function getTimeEntries(
  ...
  const urlParameters = objectToUrlSearchParameters(
    parameters as unknown as Record<string, string | number>
  )

  const requestUrl = `${
    credentials.base_api_url
  }/v1/TimeManagement/${viewId}/${templateId}?${urlParameters.toString()}`
  ...
  return (await response.json()) as GetTimeEntries_TimeEntry[]
}
